...
{
    public partial class xfrmAsistentePesajeV2 : DevExpress.XtraEditors.XtraForm
    {
        Int64 ordenID = 0;
        ...
        public xfrmAsistentePesajeV2(Int64 id_orden, int id,int batchPlanP)
        {
            InitializeComponent();
            ordenID = id_orden;
            PesajeID = id;
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (HayMPSeleccionada())
                {
                    ...
                    foreach (var item in dsMicros.New_Pesaje)
                    {
                        ...
                        if (item.seleccionar == true)
                        {
                            ...
                            if (item.id_tipo_pesaje == 0)
                            {
                                CajaDialogo.Error("DEBE SELECCIONAR EL TIPO DE PESAJE");
                                return;
                    ...
                    using (SqlConnection cnx = new SqlConnection(dp.ConnectionStringAPMS))
                    {
                        ...
                        foreach (var item in dsMicros.New_Pesaje)
                        {
                            if (item.seleccionar == true)
                            {
                                ...
                                SqlCommand cmd = new SqlCommand("dbo.sp_insert_MP_Pesaje_IndividualV3", cnx);
                                cmd.CommandType = CommandType.StoredProcedure;
                                ...
                                cmd.Parameters.Add("@id_tipo_pesaje", SqlDbType.Int).Value = item.id_tipo_pesaje;
                ...
                {
                    CajaDialogo.Error("NO SE HA CONFIGURADO NINGUNA MATERIA PRIMA");
                    this.Close();
                }
        ...
        private void LoadTipoPesaje()
        {
            ...
            try
            {
                ...
                {
                    cnx.Open();
                    dsMicros.tipo_pesaje.Clear();
                    SqlCommand cmd = new SqlCommand("dbo.sp_get_Tipo_Pesaje", cnx);
                    cmd.CommandType = CommandType.StoredProcedure;
                    SqlDataAdapter da = new SqlDataAdapter(cmd);

                    da.Fill(dsMicros.tipo_pesaje);
                    cnx.Close();
