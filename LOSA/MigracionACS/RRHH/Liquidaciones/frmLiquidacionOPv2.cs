...
{
    public partial class frmLiquidacionOPv2 : DevExpress.XtraEditors.XtraForm
    {
        ...
        /// <summary>
        /// Indica la moneda del Salario. Ejemplo: 1=Lempiras     2=USD - Basicamente indica la Categoria del Empleado 2 es Categoria Confidencial.
        /// </summary>
        ...
        public frmLiquidacionOPv2(UserLogin pUserLogin)
        {
            InitializeComponent();
            UsuarioLogeado = pUserLogin;
            LoadPorcentajes();
            dtEgreso.DateTime = dp.Now();
        }
        ...
        private void cmdBuscarEmpleado_Click(object sender, EventArgs e)
        {
            ...
            tsAplicaPreaviso.Toggled += new EventHandler(tsAplicaPreaviso_Toggled);
            cmdBuscarEmpleadoFN();
            CargarLiquidacionHeader(idEmpleado, IdContrato);
        ...
        private void cmdBuscarEmpleadoFN()
        {
            try
            {
                ...
                frmSearchEmployee frm = new frmSearchEmployee(frmSearchEmployee.TipoBusqueda.ListaEmpleados, 0);
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    HrEmployee hr = new HrEmployee();
                    hr.RecuperarDatosPorCodigo(frm.ItemSeleccionado.ItemCode);
                    IdEmpleado = hr.Id;
                    IdContrato = hr.ContractId;
                    txtNombre.Text = NombreEmpleado = hr.Name;
                    txtCodigo.Text = CodigoEmpleado = hr.Barcode;

                    Empleado emp1 = new Empleado();
                    if (emp1.RecuperarDatosForLiquidacionRRHH(IdEmpleado))
                    {
                        int idContract = hr.ContractId;
        ...
        private void CalculoDeTotalResumen()
        {
        ...
        private void CalcularEdad()
        {
            ...
            try
            {
                ...
                if (dr.Read())
                {
                    ...
                    //txtAntiguedad.Text = GetAntiguedad(dtIngreso.DateTime, dtEgreso.DateTime);
        ...
        private string GetAntiguedad(DateTime dtIngreso, DateTime dtEgreso)
        {
            string Antiguedad = "";
            int Anio = 0;
            int Meses = 0;
            ...
            //Sacar Años
            Anio = dtEgreso.Year - dtIngreso.Year;

            if (dtEgreso < dtIngreso.AddYears(Anio))
                --Anio;

            //Sacar Meses

            int Anios = dtEgreso.Year - dtIngreso.Year;
            if (dtEgreso < dtIngreso.AddYears(Anios))
            {
                Meses = dtIngreso.AddYears(Anios).Month - dtEgreso.Month;
                int dif = Math.Abs((dtIngreso.AddYears(Anio).Month - dtEgreso.Month) + 12 * (dtIngreso.AddYears(Anio).Year - dtEgreso.Year));
                if (dif == 0)
                    Meses = 11;
                else
                    Meses = 12 - dif;
            ...
            if (dtEgreso < dtIngreso.AddYears(Anio))
            {
                Dias = dtIngreso.AddYears(Anio).Day - dtEgreso.Day;
                int dif = Math.Abs((dtIngreso.AddYears(Anio).Day - dtEgreso.Day) + 12 * (dtIngreso.AddYears(Anio).Month - dtEgreso.Month));
                if (dif == 0)
                    Meses = 29;
                else
                    Meses = 30 - dif;
            ...
            Antiguedad = Convert.ToString(Anio + " Año(s)" + Meses + " Mes(es)" + Dia + "Dia(s)");
        ...
        private void CargarLiquidacionHeader(int pIdEmpleado, int pIdContrato)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@id_contrato", pIdContrato);
                SqlDataReader DR = cmd.ExecuteReader();
                if (DR.Read())
                {
        ...
        private void LoadPorcentajes()
        {
            ...
            try
            {
                ...
                if (Id_liquidacion_ == 0)
                {
                    foreach (dsRRHH_.porcentajes_liquidacionRow item in dsRRHH_1.porcentajes_liquidacion)
                    {
                        if (item.id == 4)
        ...
        private void cmdImprimir_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                rptLiquidacion report = new rptLiquidacion(IdEmpleado, liqui, Anio, IdContrato) { DataSource = dsRRHH_1, DataMember = "deducciones", ShowPrintMarginsWarning = false };
                report.CreateDocument();
                for (int i = 0; i < 1; i++)
                {
                    subrReportUltimosMesesxVaca report2 = new subrReportUltimosMesesxVaca(IdEmpleado, Anio, DiasTomados, IdContrato)
        ...
        private void GuardarLiquidacion()
        {
            ...
            using (SqlConnection connection = new SqlConnection(dp.ConnectionStringCostos))
            {
                ...
                try
                {
                    ...
                    command.CommandText = "sp_insert_RRHH_liquidacion_hV2";
        ...
        private void gridView3_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (row.id == -2)
            {
                ...
                DiasVacaciones_A_pagar_en_liquidacion = row2.dias - DiasTomados;// 
