...
namespace LOSA.TransaccionesMP
{
    public partial class frmAsjuteInventarioPorLote : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmAsjuteInventarioPorLote(UserLogin pUserLogin, int pIdMP,string pLote, int pIdBodega, int TypeId, int pIdLoteExterno)
        {
            //Esta Ruta proviene de Reporte Kardex Inventario General.
            InitializeComponent();
            ...
            gridLookUpEditOrigen.EditValue = pIdBodega;
            txtNumLote.Text = pLote;
            ...
            MostrarExterno = true;
            LoadMaestrosBodegas();
            //gridLookUpEditOrigen.Enabled = false;
            gridLookUpEditPresentacion.Enabled = false;
            radioLoteExistente.Checked = true;
            radioLoteNuevo.Enabled = false;

            //Debemos Buscar IdLoteExterno - IdPresentacion
            LoteExterno ext = new LoteExterno();
            //ext.RecuperarInfoLotesExternos(pIdMP, pLote);
            IdLoteExterno = pIdLoteExterno;
            gridLookUpEditPresentacion.EditValue = ext.IdPresentacion;
                ...
                if (radioLoteExistente.Checked)
                {
                    if (MostrarExterno == true)
                    {
                        try
                        {
                            ...
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.Parameters.AddWithValue("@entrada", spinEditPesoKg.EditValue);
                            cmd.Parameters.AddWithValue("@salida",0);
                            cmd.Parameters.AddWithValue("@lote", txtNumLote.Text);
                            cmd.Parameters.AddWithValue("@id_mp", Id_MP);
                            cmd.Parameters.AddWithValue("@itemcode", ItemCode);
                            cmd.Parameters.AddWithValue("@id_bodega", gridLookUpEditOrigen.EditValue);
                            cmd.Parameters.AddWithValue("@user_id", UsuarioLogueado.UserId);
                            cmd.Parameters.AddWithValue("@ud_entrada", spinEditUnidades.EditValue);
                            cmd.Parameters.AddWithValue("@ud_salida", 0);
                            cmd.Parameters.AddWithValue("@id_lote_externo", IdLoteExterno);
                            cmd.ExecuteNonQuery();
                            conn.Close();
                ...
                if (radioLoteExistente.Checked)
                {
                    if (MostrarExterno == true)
                    {
                        try
                        {
                            ...
                            cmd.Parameters.AddWithValue("@salida", spinEditPesoKg.EditValue);
                            cmd.Parameters.AddWithValue("@lote", txtNumLote.Text);
                            cmd.Parameters.AddWithValue("@id_mp", Id_MP);
                            cmd.Parameters.AddWithValue("@itemcode", ItemCode);
                            cmd.Parameters.AddWithValue("@id_bodega", gridLookUpEditOrigen.EditValue);
